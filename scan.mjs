...
import { appendDecisionLog, decisionLogRecord, evaluateCandidate, summarizeDecisions } from './lib/scan-decision.mjs';
...
// ── Posting-age filter ──────────────────────────────────────────────
// The day count comes from resolveConfiguredMaxAgeDays (scan_max_age_days,
// then max_posting_age_days). Absent or invalid → every offer passes.
// An offer is skipped only when the provider supplied a postedAt (epoch ms)
// AND it is older than N days. Offers with no date always pass.
...
// UI scans read `scan_max_age_days`. Older CLI configs used `max_posting_age_days`.
// The shared window is the first positive integer of those two, in that order.
export function resolveConfiguredMaxAgeDays(config = {}) {
  const scanMax = Number(config?.scan_max_age_days);
  if (Number.isInteger(scanMax) && scanMax > 0) return scanMax;
  const legacy = Number(config?.max_posting_age_days);
...
/**
 * A labeled trailing segment of a pipeline entry, as written by the writers
 * that emit one: `formatPipelineOffer` here (`posted:`, `trust:`, `note:`),
 * `appendRankAnnotation` in `rank-pipeline.mjs` (`rank:`), and the UI record
 * writer (`source:`, `jev:`, `remote_verdict:`). `remote:` is not a label:
 * `Remote: EMEA` is a real location cell.
 *
 ...
 *
 * Case-insensitive for the labels writers emit, except `remote:`. A live
 * location is often `Remote: EMEA`; only the lowercase writer tag is metadata.
 */
const PIPELINE_LABELED_SEGMENT_RE = /^(?:posted|trust|note|rank|source|jev|remote_verdict):\s/iu;
const PIPELINE_REMOTE_LABEL_RE = /^remote:\s/u;

function isPipelineLabeledSegment(cell) {
  return PIPELINE_LABELED_SEGMENT_RE.test(cell) || PIPELINE_REMOTE_LABEL_RE.test(cell);
}
...
function extractPipelineCompanyRole(line) {
  ...
  const location = urlIndex === 0 && !isPipelineLabeledSegment(third) ? third : '';
...
export function loadDedupSnapshot(policy = {}, canonicalize = defaultCompanyNormalizer, {
  ...
  for (const line of scanHistoryText.split('\n').slice(1)) {
    ...
    const key = normalizeUrlForDedup(url);
    const iso = String(firstSeen).slice(0, 10);
    const prev = firstSeenByUrl.get(key);
    if (!prev || iso < prev) firstSeenByUrl.set(key, iso);
  }
  return { seen, recheckEligible, seenCompanyRoles, seenCompanyRoleBases, fingerprintHistory, firstSeenByUrl };
}
...
// The append-only column set formatScanHistoryRow() emits, in order: the
// original 7 positional cols (url…location) plus the trailing cols added
// since — fingerprint (7), posted_at (8), trust_score (9), trust_flags (10),
// normalized_company (11). Exported (rather than inlined below) so
// verify-scan-history.mjs (local to this fork, not upstream) validates row
// width against this same constant instead of a hardcoded column count — the
// two drifted apart before (a schema widening here silently made every
// current-format row look "malformed" to a verifier still checking for 6).
...
export async function appendToScanHistory(offers, date, status = 'added') {
  await withPipelineLock(SCAN_HISTORY_PATH, () => {
    // Written ONLY on fresh-file creation; existing files (including
    // headerless legacy files and older narrower-header files) are never
    // rewritten. All readers either skip line 0 unconditionally, detect the
    // header by its `url\t` prefix, or skip non-URL col-0 rows, so widening
    // it stays backward-compatible. `status` is parameterized so callers can
    // record verify outcomes (`skipped_expired`, etc.) without the legacy
    // `(expired)` suffix.
    if (!existsSync(SCAN_HISTORY_PATH)) {
      mkdirSync(path.dirname(SCAN_HISTORY_PATH), { recursive: true });
      atomicWriteFile(SCAN_HISTORY_PATH, SCAN_HISTORY_HEADER);
    }
...
async function main() {
  ...
  const maxAgeDays = resolveConfiguredMaxAgeDays(config);
  const postingAgeFilter = buildPostingAgeFilter(maxAgeDays);
  const postedDateFilter = buildPostedDateFilter(effectiveAfter, postedBefore);

  // Same bound the filter above uses, widened by the configured max age when set.
  // Derived by the same helper so the hint and the filter cannot disagree.
  const earlyStopSinceMs = resolveEarlyStopMs(effectiveAfter, maxAgeDays);
  ...
  const firstSeenByUrl = dedupSnapshot.firstSeenByUrl ?? new Map();
  ...
  const recordSkip = (job, company, reasonCode, reason) => {
    decisionRecords.push(decisionLogRecord({
      ...job,
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        if (blacklist.size > 0) {
          ...
          if (blEntry) {
            if (!includeBlacklisted) {
              ...
              recordSkip(job, company, 'blacklist', blEntry.reason || 'company blacklist');
              continue;
            }
        ...
        const decision = evaluateCandidate({
          ...
          locationAllows: () => locationFilter(job.location, job.url, job.title),
          countryAllows: () => countryEligibilityFilter(job.description),
          today: date,
          firstSeenFor: (candidate) => firstSeenByUrl.get(normalizeUrlForDedup(candidate.url)) || '',
        });
        if (decision.disposition === 'reject') {
          ...
          if (decision.reasonCode === 'remote') totalFilteredRemote++;
          else if (decision.reasonCode === 'location') totalFilteredLocation++;
          else if (decision.reasonCode === 'country') totalFilteredCountryEligibility++;
          else if (decision.reasonCode === 'age') totalFilteredPostingAge++;
          else totalFilteredTitle++;
          continue;
        }
        job.remoteDisposition = decision.remote.disposition;
        job.remoteConfidence = decision.scores.remote;
        job.preScore = decision.scores.overall;
        job.dateSource = decision.freshness.source;
        if (decision.disposition === 'review') job.note = job.note ? `remote:review — ${job.note}` : 'remote:review';
        if (decision.freshness.source === 'proxy') job.note = job.note ? `${job.note} — date:proxy` : 'date:proxy';
        if (classifyTier && skipTiers.includes(classifyTier(job.title))) {
          totalFilteredTier++;
          recordSkip(job, company, 'tier', `tier ${classifyTier(job.title)} is skipped`);
          continue;
        }
        if (!postingAgeFilter(job.postedAt)) {
          totalFilteredPostingAge++;
          recordSkip(job, company, 'age', 'max_posting_age_days');
          continue;
        }
        if (!postedDateFilter(job.postedAt)) {
          totalFilteredPostedDate++;
          recordSkip(job, company, 'posted_date', 'posted date filter');
          continue;
        }
        if (!salaryFilter(job.salary)) {
          totalFilteredSalary++;
          recordSkip(job, company, 'salary', 'salary filter');
          continue;
        }
        if (!contentFilter(job.description, matchedTitleKeywords(job.title, config.title_filter))) {
          totalFilteredContent++;
          recordSkip(job, company, 'content', 'content filter');
          continue;
        }
        if (!visaFilter(job.description)) {
          totalFilteredVisa++;
          recordSkip(job, company, 'visa', 'visa filter');
          continue;
        }
        const dedupUrl = normalizeUrlForDedup(job.url);
        if (seenUrls.has(dedupUrl)) {
          totalDupes++;
          recordSkip(job, company, 'duplicate', 'url already seen');
          continue;
        }
        ...
        if (
          ...
          totalDupes++;
          recordSkip(job, company, 'duplicate', 'company and title already seen');
          continue;
        }
        const cooldownResult = cooldownFilter(job);
        if (cooldownResult.skip) {
          totalFilteredCooldown++;
          recordSkip(job, company, 'cooldown', cooldownResult.reason || 're-apply cooldown');
          cooldownOffers.push({
  ...
  if (config.remote_filter || totalFilteredRemote > 0) {
    console.log(`Filtered by remote:    ${totalFilteredRemote} removed`);
  }
  if (skipTiers.length > 0) {
  ...
  if (maxAgeDays != null || totalFilteredPostingAge > 0) {
    console.log(`Filtered by age:       ${totalFilteredPostingAge} removed${maxAgeDays != null ? ` (max ${maxAgeDays}d)` : ''}`);
  }
  ...
  const decisionSummary = summarizeDecisions(decisionRecords);
  const decisionParts = Object.entries(decisionSummary).map(([key, count]) => `${key}=${count}`);
  if (decisionParts.length) console.log(`Decision reasons:      ${decisionParts.join(', ')}`);
  if (!dryRun && decisionRecords.length) {
    appendDecisionLog(SCAN_DECISIONS_PATH, decisionRecords);
    console.log(`Decision log:          ${SCAN_DECISIONS_PATH}`);
  } else if (dryRun && decisionRecords.length) {
    console.log('Decision log:          not written (--dry-run)');
  }
