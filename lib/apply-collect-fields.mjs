...
export const COLLECT_FIELDS = () => {
  ...
  for (const el of document.querySelectorAll(
    'input, textarea, select, [contenteditable="true"], [role="textbox"], [role="checkbox"], [role="radio"], [role="switch"]',
  )) {
    ...
    // Ashby stores both Yes and No in one hidden checkbox. Checked=false
    // cannot distinguish No from unanswered; the visible buttons can.
    const yesNoWrapper = type === 'checkbox' ? el.closest('.ashby-application-form-input-yesno') : null;
    const yesNoButtons = yesNoWrapper
      ? [...yesNoWrapper.querySelectorAll('button[data-option][aria-pressed]')].filter(visible) : [];
    if (yesNoButtons.length === 2 && yesNoButtons.some(b => b.dataset.option === 'yes')
      && yesNoButtons.some(b => b.dataset.option === 'no')) type = 'radio';
