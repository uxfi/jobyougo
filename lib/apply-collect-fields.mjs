...
export const COLLECT_FIELDS = () => {
  ...
  for (const el of deepQueryAll(document,
    'input, textarea, select, [contenteditable="true"], [role="textbox"], [role="combobox"], [role="checkbox"], [role="radio"], [role="switch"], button[aria-haspopup="listbox"], button[aria-haspopup="menu"], [role="button"][aria-haspopup="listbox"]',
  )) {
    ...
    const isPopupList = popup === 'listbox' || popup === 'menu' || popup === 'true';
    ...
    else if (role === 'combobox' || isPopupList) type = 'text';
    else type = isCE ? 'textarea' : (el.type || tag).toLowerCase();
    if (isPopupList && (tag === 'button' || role === 'button')) {
      const blob = (el.innerText || el.getAttribute('aria-label') || '').replace(/\s+/g, ' ').trim();
      if (/^(apply|submit|next|continue|postuler|candidater|sign[\s-]?in|log[\s-]?in)\b/i.test(blob)) continue;
    }
    if (!isPopupList && role !== 'combobox' && role !== 'checkbox' && role !== 'radio' && role !== 'switch'
