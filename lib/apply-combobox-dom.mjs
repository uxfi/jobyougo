...
export function COMBOBOX_OPTION_QUERY({ mode, i, want, allowGlobal }) {
  ...
  if (el) {
    ...
    if (owned.length) {
      ...
      // Symmetric above/below: a menu near the bottom of the viewport (very
      // common on a long application form) routinely flips UPWARD to stay on
      // screen — react-select/Radix/MUI etc. all do this. The old window only
      // allowed options as far as 16px above the field's own top, so an
      // upward-flipped menu (its rows sitting well above that line) was never
      // "near" and, for anything not already flagged a known combobox
      // (allowGlobal=false), fell through to typing raw text into the field
      // instead of clicking a real option.
