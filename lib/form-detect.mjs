...
export const APPLICATION_FORM_PROBE = () => {
  ...
  const fileProxyVisible = (el) => {
    ...
    for (let d = 0; d < 4 && node; d++, node = node.parentElement) {
      ...
      if (!t) continue;
      // A page-level ancestor that happens to contain dropzone copy from a
      // hidden Application tab is not a visible upload widget.
  ...
  const applyEntryCta = [...document.querySelectorAll('a, button, [role="button"]')]
    .filter(visible)
  ...
  const hasVisibleField = inputs.length > 0 || usableFiles.length > 0
    || [...document.querySelectorAll('select, [role="combobox"]')].some(el => visible(el) && !inClosedTab(el));
