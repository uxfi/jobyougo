...
const NAV_TEXT_RE = /^(jobs?|careers?|open (roles|positions)|all jobs|view (all|role)|see all|search|browse|apply|learn more|read more|back|next|previous|home|life at|why join|benefits|culture|teams?|departments?|locations?|filters?|sign in|log ?in|skip to|opens in a new)/i;
...
export async function scrapeCareersPage(careersUrl, companyName, opts = {}) {
  const { timeoutMs = 45000, maxResults = 120, maxPages = 4, headless = true } = opts;
  let browser;
  try {
    ...
    // Wait for a posting-shaped link, not just any anchor. Navigation links are
    // often present before client-rendered job cards, and extracting at that
    // point silently misses slow SPA listings.
    try {
      await page.waitForFunction(
        ({ jobHrefSrc }) => {
          const jobHref = new RegExp(jobHrefSrc, 'i');
          return Array.from(document.querySelectorAll('a[href]')).some(a => jobHref.test(a.href));
        },
        { jobHrefSrc: JOB_HREF_RE.source },
        { timeout: Math.min(10000, Math.max(4000, Math.floor(timeoutMs / 3))) },
      );
      await page.waitForTimeout(500);
    } catch { /* no posting-shaped anchors — fall through and return [] */ }

    // Human-like discovery: allow lazy lists to settle, scroll through the
    // current result page, then follow a visible next/load-more control. This
    // covers SPA boards where the target role is not present in the first DOM.
    for (let pageNo = 0; pageNo < maxPages; pageNo++) {
      ...
      const advanced = await page.evaluate(() => {
        ...
        const el = candidates.find((x) => {
          const t = String(x.innerText || x.getAttribute('aria-label') || '').trim();
          const disabled = x.hasAttribute('disabled') || x.getAttribute('aria-disabled') === 'true';
          return !disabled && /^(next|load more|show more|more jobs|older|view more)$/i.test(t);
        });
        if (!el) return false;
    ...
    const rows = await page.evaluate(({ jobHrefSrc, navTextSrc, cap }) => {
      ...
      for (const a of document.querySelectorAll('a[href]')) {
        ...
        if (card && (!text || navText.test(text))) {
