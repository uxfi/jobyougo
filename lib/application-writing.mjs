...
- One relevant documented example beats a catalogue. Pick the employer or client reference by brand weight (LVMH, Renault, Société Générale first) and tenure (multi-year before a 6-month role or a prototype). OneAsset is current work, not the default if a stronger or longer brand fits. Do not invent admiration, usage, emotion, employers, tools or results.
- Personal projects (UXfi, Flemme OS, Creads.io, Panfy, Jarvos, JobYouGo, Ancient World) are NOT references. Do not use them to prove experience, skill, or AI/LLM work. Motivation only: one short clause of interest, never "On Creads.io…" / "At Flemme OS…" as the proof.
- AI / LLM questions: the reference is the employer workflow (OneAsset: Cursor, Claude, GitHub). Concrete practices (API, prompt design, validation pass, structured JSON, RAG, agent steps). No buzzword stack. No personal-product name as the credential. Figmol is OneAsset's internal tool for reviewing the live app. Do not list it as a tool next to Cursor, Claude, GitHub, or Figma. Name it only inside the OneAsset story.
...
- Choose the detail that explains this answer. Prefer process over a scored result. One employer or client reference, ranked by brand importance and tenure. Personal projects are motivation support only, never the reference.
...
const SIDE_PROJECT_NAMES =
  'UXfi|Flemme(?:\\s+OS)?|Creads(?:\\.io)?|Panfy|Jarvos|JobYouGo|Ancient\\s+World';

// "On Creads.io…" / "Chez Flemme…" as if the product were a known employer/brand.
const SIDE_PROJECT_BRAND_OPEN_RE = new RegExp(
  `^\\s*(?:On|At|Sur|Chez)\\s+(${SIDE_PROJECT_NAMES})\\b[,:]?\\s*`,
  'i',
);

function debrandSideProjectOpen(text) {
  const raw = String(text || '');
  const m = raw.match(SIDE_PROJECT_BRAND_OPEN_RE);
  if (!m) return raw;
...
export function sanitizeApplicationProse(value) {
  ...
  const paragraphs = replaceClauseDashes(out).split(/\n{2,}/).map((para) => {
    let p = debrandSideProjectOpen(para);
