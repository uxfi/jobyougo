...
const EU_SIGNALS = /\b(europe|eu\b|emea|paris|france|cet\b|cest\b|berlin|germany|london|uk\b|united\s*kingdom|amsterdam|netherlands|madrid|spain|lisbon|portugal|dublin|ireland|utc\s*\+\s*[012]\b|gmt\s*\+\s*[012]\b|european)\b/i;
...
export async function buildApplySpec({ root, reportFilename, company, role, region: regionOverride, autoSubmit = false }) {
  ...
  const profile = yamlLoad(await readFile(join(root, 'config', 'profile.yml'), 'utf-8'));
  ...
  return {
    ...
    identity: {
      ...
      phone: (region === 'asia' ? candidate.phone_asia : candidate.phone) || candidate.phone || '',
