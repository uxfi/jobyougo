...
const REPLACEMENTS = [
  ...
  // Stock AI transition-word openers — strip the connective, keep the sentence.
  [/^\s*(?:furthermore|moreover|additionally|in\s+addition)[,:]?\s*/gim, ''],
  [/^\s*(?:in\s+conclusion|overall|in\s+summary|to\s+summarize)[,:]?\s*/gim, ''],
  [/^\s*that\s+said[,:]?\s*/gim, ''],
  [/\bin\s+today'?s\s+(fast-paced|rapidly\s+evolving|competitive)\s+[a-z\s]+?,\s*/gi, ''],
  [/\bit'?s\s+worth\s+noting\s+that\s+/gi, ''],
  [/\bplays?\s+a\s+(crucial|critical|vital|key)\s+role\s+in\b/gi, 'matters for'],
  [/\bat\s+the\s+end\s+of\s+the\s+day\b/gi, ''],
  [/\bdelve\s+into\b/gi, 'dig into'],
  [/\bshowcas(e|ing|ed)\b/gi, 'show$1'],
  [/\bboasts?\b/gi, 'has'],
  [/\brobust\s+solution\b/gi, 'solution'],
  // Model self-reference / meta-commentary that must never reach a real form.
  [/\bas an ai( language model)?\b,?\s*/gi, ''],
  [/^\s*(?:sure|certainly|of course|absolutely)[,!]\s*(?:here'?s?|i'?ll|i can)\b[^.]*?:\s*/gi, ''],
];
...
// URLs, emails, and bare domains/handles ("github.com/uxfi") must never be
// sentence-cased — this same polish function also runs over raw identity
// field values (LinkedIn, GitHub, portfolio links), not just prose answers,
// and "https://…" → "Https://…" silently breaks the link.
function looksLikeRawValue(text) {
...
function normalizeSentenceStarts(text) {
  if (looksLikeRawValue(text)) return text;
  const withInnerCaps = text.replace(/([.!?]\s+)([a-z])/g, (_, end, letter) => end + letter.toUpperCase());
  // Stripping a leading tell ("Furthermore,", "In today's...") can leave the
  // very first letter lowercase — fix the string's own start too, not just
  // sentences after a period (the regex above never touches position 0).
  return withInnerCaps.replace(/^([a-z])/, (_, letter) => letter.toUpperCase());
}
...
export function polishApplicationAnswer(value) {
  ...
  text = text
    ...
    // Numeric ranges are collapsed to a plain hyphen FIRST so the dash-as-
    // punctuation rules below can't reach them: "EUR 70–110K" turning into
    // "EUR 70, 110K" made the salary answer nonsense on a live Ashby form,
    // and the same applied to date ranges ("2020–2024") and rates.
