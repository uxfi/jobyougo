...
export const CHALLENGE_HTML_MARKERS = [
  ...
  '/akam/',
  // Vendors apply-runner.mjs already detected via DOM selectors; kept in sync
  // so the HTTP path and the browser path recognize the same walls.
...
export const CHALLENGE_TEXT_MARKERS = [
  ...
  'anomaly-modal',
  // Phrases apply-runner.mjs already matched; kept in sync so a wall the
  // browser path recognizes is not missed by the HTTP path. Only the ones
  // distinctive enough to be safe on a long page live here — the generic
  // half is in WEAK_TEXT_MARKERS below.
 ...
 *
 * The anti-bot vocabulary below overlaps heavily with the vocabulary of the
 * roles this scanner targets: a Trust & Safety PM posting genuinely says
 * "human verification", "automated access", "additional verification required".
 * As strong markers they blocked exactly the postings the user cares most
 * about, so they are length-gated instead.
 */
