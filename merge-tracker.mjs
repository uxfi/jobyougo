...
function validateStatus(status) {
  ...
  // Aliases (mapping old Spanish/French and current variations to English canonical states)
  const aliases = {
    // English
    'applied': 'Applied', 'sent': 'Applied',
    'evaluated': 'Evaluated', 'evaluated (a)': 'Evaluated', 'evaluated (b)': 'Evaluated',
    'responded': 'Responded',
    'interview': 'Interview', 'interviews': 'Interview',
    'offer': 'Offer', 'offered': 'Offer',
    'rejected': 'Rejected', 'discarded': 'Discarded',
    'no apply': 'SKIP', 'no_apply': 'SKIP', 'skip': 'SKIP', 'monitor': 'SKIP',
    
    // Spanish/legacy
    'evaluada': 'Evaluated', 'evaluar': 'Evaluated',
    ...
    'no aplicar': 'SKIP', 'no_aplicar': 'SKIP', 'geo blocker': 'SKIP',

    // Logic
    'condicional': 'Evaluated', 'hold': 'Evaluated', 'verificar': 'Evaluated',
  };

  if (aliases[lower]) return aliases[lower];

  // DUPLICADO/Repost → Discarded
  if (/^(duplicado|dup|repost|duplicate)/i.test(lower)) return 'Discarded';

  console.warn(`⚠️  Status "${status}" mapped to Evaluated (default)`);
  return 'Evaluated';
}
