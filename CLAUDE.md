...
### Report Integrity (CRITICAL)

**NEVER write raw tool_call, tool_response, browser_snapshot JSON, or internal reasoning into a report file.**

Report files in `reports/` MUST contain ONLY structured evaluation content (A-F blocks with real analysis). Before writing a report:

1. **Complete the FULL evaluation in memory first** — all A-F blocks with scores
2. **Write the file in a single operation** — never incrementally append
3. **Validate the content** — ensure it has a real Score (X.X/5, not `—`), at least 2 sections, and NO raw logs
4. **Never create a placeholder report** — if the evaluation isn't done, don't create the file

Corrupted report = contains any of: `<tool_call>`, `<tool_response>`, `browser_snapshot`, `browser_navigate`, `</thinking>`

To detect and fix corrupted reports: `node verify-reports.mjs --fix`
...
6. Report validation: `node verify-reports.mjs` (and `--fix` to clean corrupted)
...
### Deleting an Application (CRITICAL)

**NEVER simply remove a row from `applications.md` without registering the deletion.** Otherwise the scanner will re-discover and re-add the offer.

**When the user asks to delete / remove an application:**

1. Note the application's URL (from the report or pipeline.md if available)
2. Append a row to `data/scan-history.tsv`:
   ```
   {url}\t{today}\tmanual-delete\t{role}\t{company}\tdeleted
   ```
   If the URL is unknown, use `unknown:{company-slug}:{role-slug}` as a placeholder.
3. Append a row to `data/deleted-applications.tsv` (create if missing with header `company\trole\tdate_deleted\treason`):
   ```
   {company}\t{role}\t{YYYY-MM-DD}\t{reason or "user_deleted"}
   ```
4. **Set status to `Discarded` in `applications.md`** — do NOT remove the row. This keeps the offer visible in the tracker for reference while preventing future scans from re-adding it.
5. Sync the status change to Supabase:
   ```bash
   node sync-supabase.mjs applications
   ```
6. Optionally delete the associated report in `reports/` if the user explicitly asks.

This ensures future scans will skip this offer permanently (both URL-based and company+role-based dedup), while keeping the offer visible in the tracker with status `Discarded`.
