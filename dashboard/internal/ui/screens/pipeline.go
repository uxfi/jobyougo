...
// PipelineAnswerQuestionMsg is emitted when the user wants to answer an application question for the selected job.
type PipelineAnswerQuestionMsg struct {
	Company string
	Role    string
}
...
var statusOptions = []string{"Evaluated", "Applied", "Responded", "Rejected", "Discarded", "SKIP"}
...
func (m PipelineModel) handleKey(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	...
	case "a":
		if app, ok := m.CurrentApp(); ok {
			return m, func() tea.Msg {
				return PipelineAnswerQuestionMsg{Company: app.Company, Role: app.Role}
			}
		}
...
func (m PipelineModel) renderHeader() string {
	...
	info := right.Render(fmt.Sprintf("%d evaluated | Avg %s/5", m.metrics.Total, avg))
...
func (m PipelineModel) renderBody() string {
	if len(m.filtered) == 0 {
		emptyStyle := lipgloss.NewStyle().
			Foreground(m.theme.Subtext).
			Padding(1, 2)
		return emptyStyle.Render("No applications match this filter")
	}
...
func (m PipelineModel) renderAppLine(app model.CareerApplication, selected bool) string {
	...
	roleW := m.width - scoreW - companyW - statusW - compW - btnW - 10
	if roleW < 15 {
		roleW = 15
	}
	...
	// Question button — highlighted on selected row
	var btnStyle lipgloss.Style
	if selected {
		btnStyle = lipgloss.NewStyle().Bold(true).Foreground(m.theme.Base).Background(m.theme.Mauve)
	} else {
		btnStyle = lipgloss.NewStyle().Foreground(m.theme.Subtext)
	}
	btn := btnStyle.Render("[?]")
	...
	if selected {
		selStyle := lipgloss.NewStyle().
			...
			Width(m.width - 4 - btnW)
		return padStyle.Render(selStyle.Render(line) + " " + btn)
	}
	return padStyle.Render(line + " " + btn)
}
